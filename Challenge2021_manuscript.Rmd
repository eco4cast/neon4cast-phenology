...
jpeg(file="exampleForecastsOnDay.jpeg",width = 8.57, height = 6.01, units = "in",res = 1000)
...
jpeg(file="exampleForecastsOnDay_Harvard.jpeg",width = 9.5, height = 6, units = "in",res = 1000)
multi_team_plot(combined_forecasts = gcc_forecast_subset, 
                target = "gcc_90", 
                theme = "phenology", 
                date = startDate, 
  ...
  for(site_number in all_site_number){
    ...
    for(t in c(2,5,8)){ #Loops over the transition dates
      ...
      if(multipleSites){
        ...
        plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=ylims,ylab="",xlab="Days Before Transition Date",main=tle,bty="n")
      }
      if(statistic=="GCC"){
...
jpeg(file="forecastedValuesOfTransitionDates_rescaled.jpeg",width = 9, height = 5, units = "in",res = 1000)
...
jpeg(file="forecastedValuesOfTransitionDates.jpeg",width = 9, height = 5, units = "in",res = 1000)
...
jpeg(file="CRPSvsLeadTimeTrans.jpeg",width = 9, height = 5, units = "in",res = 1000)
...
jpeg(file="BiasvsLeadTimeTrans.jpeg",width = 9, height = 5, units = "in",res = 1000)
...
jpeg(file="SDvsLeadTimeTrans.jpeg",width = 9, height = 5, units = "in",res = 1000)
...
##Extra Figure. Calculate the lead times that each model and site performed better than climatology (lower CRPS) for different transition dates and Supplementary Figure 
```{r}
...
meta$class[meta$class=="TRUEFALSE"] <- "covariate"
meta$class[meta$class=="FALSEFALSE"] <- "static"
meta$class[meta$class=="TRUETRUE"] <- "dynamic"
meta$class[meta$class=="FALSETRUE"] <- "persist"
}
...
```{r}
jpeg(file="GAM_TeamsComparedToClim.jpeg",width = 8, height = 5, units = "in",res=1000)
ggplot(beta.team) + 
 ...
 geom_errorbar( aes(x=team, ymin=Estimate-1.96*SE, ymax=Estimate+1.96*SE), width=0.4, colour="orange", alpha=0.9, size=1.3) +
  ...
  theme(legend.position = c(0.8, 0.2), panel.background = element_blank()) +
...
jpeg(file="GAM_siteCRPS.jpeg",width = 7, height = 4, units = "in",res=1000)
ggplot(beta.site) + 
...
## visualize
jpeg(file="GAM_interactions.jpeg",width = 5, height = 8, units = "in",res=1000)
...
```{r}
jpeg(file="GAM_responseSurfaces.jpeg",width = 4, height = 8, units = "in",res=1000)
layout(matrix(c(1,2,3,4,4),ncol=1,byrow=TRUE))
par(mai=c(0.6,0.6,0.3,0.5))

## OVERALL
plot(hnew,crps_horiz,xlab="",ylab="Predicted CRPS",type="l",lwd=3,bty="n",xaxt="n",main="a)")
axis(1, at=seq(0,35,5),labels=rep("",8))

## MODEL TYPE: CRPS
plot(hnew,crps_horiz5a,xlab="",ylab="Predicted CRPS",
     ylim=c(0,0.07),type='l',lwd=1,lty=2,bty="n",xaxt="n",main="b)")
for(i in seq_along(mtype)){
    lines(hnew,crps_horiz4[[i]],col=classCls[i],lwd=3)
}
axis(1, at=seq(0,35,5),labels=rep("",8))
...
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="",ylab=expression(paste(Delta, "CRPS")),
     ylim=c(-0.005,0.03),type='l',lwd=1,lty=2,bty="n",xaxt="n",main="c)")
axis(1, at=seq(0,35,5),labels=rep("",8))
for(i in seq_along(mtype)){
  y = crps_horiz4[[i]]
  lines(hnew,y-y[1],col=classCls[i],lwd=3)
...
#MODEL: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="Lead Time",ylab=expression(paste(Delta, "CRPS")),
     ylim=c(-0.02,0.05),type='l',lwd=1,lty=2,bty="n",main="d)")
for(i in seq_along(toPlot5)){
  y = crps_horiz5[[toPlot5[i]]]
  lines(hnew,y-y[1],col=cls[toPlot5[i]],lwd=3)
}
