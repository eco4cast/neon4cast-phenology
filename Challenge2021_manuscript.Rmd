---
title: "EFI NEON Phenology Forecasting Challenge Round 1"
author: "EFI NEON phenology working group (Primarily Kathryn Wheeler and Michael Dietze)"
contact: "kwheelerecology@gmail.com"
...
Data Files of cleaned up forecasts, meta data, and calculated site transition dates (csv's) should be downloaded from Zenodo. Previous code for downloading and processing these are included towards the end for completeness, but they may not work as the infrastructure changes. 

Some figures were further edited outside of R for aesthetics. 

Within this code and the csv files, "team" refers to the term "model" used within the manuscript. In the EFI challenge groups registered as "teams", which got their own ID. In the manuscript we switched the terminology to "model" instead because the same group of people some times submitted multiple models. 

#Read in Data (csv's are provided on Zenodo)
```{r}
allTransitions <- read.csv(file="EFIpheno2021_allTransitions.csv")
gcc_forecasts <- read.csv(file="EFIpheno2021_gcc_forecast_subset.csv")
meta <- read.csv(file="EFIpheno2021_metaData.csv")
meanTrans <- read.csv(file="EFIpheno2021_historicalMeanTransitionDates.csv") #Created in calculatePhenoCamTransitionDates.R file on the GitHub Repo https://github.com/eco4cast/neon4cast-phenology
```

#General variables related to teams and sites
```{r}
teams <- c("PhenoPhriends", "PEG_RFR2","CU_Pheno","EFI_U_P","VT_Ph_GDD",
           ...
           "Team_MODIS","Persistence","GPEDM"," DOY_Mean") #Note: DOY_Mean model has a space before it so that it is the reference model in the GAMs, which picks one alphabetically 
...
siteColorsDF <- data.frame(siteColor=siteColors,
                           site=names(siteColors))
...
figureDirectory <- "manuscriptFigures/" #Directory to save the figures
...
#Site Location Map with NPN Spring Anomaly. NPN map is provided on Zenodo. But we grabbed the si-x_leaf_anomaly_historic.tif file of the Historical Annual Spring Indices Anomaly (2021), First Leaf - Spring Index go to https://www.usanpn.org/geoserver-request-builder and select the WCS service type and GeoTiff formats. 
```{r}
...
gccForecastsYes_forecastedDate <- data.frame(matrix(ncol=n_teams,nrow=length(challengeDays))) #Data Frame to keep track of which days each model forecasted
teamOrder <- rev(c(18,16,17,15,14,13,9,8,2,12,11,10,7,1,6,5,4,3))
for(tm in 1:n_teams){
  tmDat <- gcc_forecasts[gcc_forecasts$team==teams[tm],] #Subset by team
  ...
  for(d in 1:length(challengeDays)){
    gccForecastsYes_forecastedDate[d,tm] <- challengeDays[d] %in% uniqueTimes
  }
}
colnames(gccForecastsYes_forecastedDate) <- teams

#Plot first team and then add on subsequent teams onto the graph
tm <- 18
gccForecastsYes_forecastedDate <- gccForecastsYes_forecastedDate[,teamOrder]
plot(challengeDays[gccForecastsYes_forecastedDate[,tm]],
     rep(tm,length(challengeDays))[gccForecastsYes_forecastedDate[,tm]],pch=20,
...
for(tm in 1:n_teams){
  points(challengeDays[gccForecastsYes_forecastedDate[,tm]],
         rep(tm,length(challengeDays))[gccForecastsYes_forecastedDate[,tm]],pch=20)
}

#Plot dates of submissions 
gccForecastsYes_submittedDate <- data.frame(matrix(ncol=n_teams,nrow=length(challengeDays)))
...
for(tm in 1:n_teams){
  ...
  for(d in 1:length(challengeDays)){
    gccForecastsYes_submittedDate[d,tm] <- challengeDays[d] %in% uniqueTimes
  }
}
colnames(gccForecastsYes_submittedDate) <- teams

tm <- 18
par(mai=c(1,0.1,0.3,2))
gccForecastsYes_submittedDate <- gccForecastsYes_submittedDate[,teamOrder]
plot(challengeDays[gccForecastsYes_submittedDate[,tm]],
     rep(tm,length(challengeDays))[gccForecastsYes_submittedDate[,tm]],pch=20,
     xlab="Date",ylab="",ylim=c(0,n_teams),bty="n",yaxt="n",main="",xlim=range(challengeDays),font.main=1)
title("b) Submission Dates", adj = 0)

polygon(x=c(min(tranDates),min(tranDates),max(tranDates),max(tranDates)),y=c(-0.9,n_teams+1,n_teams+1,-0.9),col="gray",border=NA)
for(tm in 1:n_teams){
  points(challengeDays[gccForecastsYes_submittedDate[,tm]],rep(tm,length(challengeDays))[gccForecastsYes_submittedDate[,tm]],pch=20)
}
dev.off()

#Figure out first submission date for each model
modelFirstSubmissions <- apply(gccForecastsYes_submittedDate,MARGIN=2,FUN=
                                 function(x){
                                   which(x)[1]})
modelFirstSubmissions <- data.frame(team=names(modelFirstSubmissions),
                                    firstSubmission=modelFirstSubmissions)
```

#Figures S1 and S2
```{r}
#Bar plot of the number of submissions per team
barDat <- gcc_forecasts %>% group_by(team) %>% summarise(n=n())
...
## Time series figures
site_number <- 1 #Limiting to 1 site for the figure but all teams forecasted all sites
...
submitted <- apply(gccForecastsYes_submittedDate[targetRows,],1,sum)
startDate <- challengeDays[as.numeric(names(which.max(submitted)))]

submissions <- tapply(gcc_forecasts$team,
                      INDEX = gcc_forecasts$start_time,
...
#'
#' @examples
multi_team_plot <- function(combined_forecasts, target, theme, date, horizon, siteID = NA, team = NA, addSD = FALSE){
  curr_theme <- theme
  ...
  combined_forecasts$max_date <- as.Date(combined_forecasts$start_time) + lubridate::days(horizon)
  ...
  if(addSD){
    if(siteID=="HARV"){
      targetData = read.csv('https://phenocam.nau.edu/data/archive/harvard/ROI/harvard_DB_0001_1day.csv',skip=22)
      targetData <- targetData %>% mutate(time=date) %>%dplyr::select(time,gcc_std)
      combined_forecasts$time <- as.character(combined_forecasts$time)
      combined_forecasts <- left_join(combined_forecasts,targetData,by=c('time'))
      combined_forecasts$time <- as.Date(combined_forecasts$time)
    }else{
      print("Error: Can only add sd for HARV")
      return()
    }
  ...
  p <- combined_forecasts %>%
    ...
    ggplot2::geom_point(aes(y = obs), color = "black", alpha = 0.4,cex=0.9) +
  ...
  if(addSD){
    p = p+ ggplot2::geom_errorbar(aes(ymin=obs-gcc_std, ymax=obs+gcc_std), width=.2,colour="black")
  }
...
#All Sites
jpeg(file=paste0(figureDirectory,"exampleForecastsOnDay.jpeg"),width = 8.57, height = 6.01, units = "in",res = 1000) #Fig. S3
multi_team_plot(combined_forecasts = gcc_forecasts,  
                target = "gcc_90", 
                theme = "phenology", 
                date = as.Date("2021-04-28"), 
                horizon = 35)
dev.off()

#Harvard Forest Only
jpeg(file=paste0(figureDirectory,"exampleForecastsOnDay_Harvard.jpeg"),width = 10, height = 6, units = "in",res = 1000) #Fig. 3
multi_team_plot(combined_forecasts = gcc_forecasts, 
                target = "gcc_90", 
                theme = "phenology", 
...
#Lead Time Figures: 
##Figure S4: Example of how teams' forecasts change as lead time decreases for two sites for 15%, 50%, and 85% thresholds
```{r}
...
plotForecastedValuesOverTime <- function(all_site_number, gcc_forecast_subset,statistic,ylims, rescaled, trans=c(2,5,8)){
  ...
  for(site_number in all_site_number){
    sitDat <- gcc_forecasts[gcc_forecasts$siteID==site_names[site_number],]
...
plotForecastedValuesOverTime(all_site_number=c(8,2),gcc_forecast_subset=gcc_forecasts,statistic = "GCC", ylims=c(0.32,0.46),rescaled=FALSE,trans=c(2,8))
dev.off()
```
...
for(s in 1:length(site_names)){
  sitDat <- gcc_forecasts[gcc_forecasts$siteID==site_names[s],] 
  sitDat_DOY_Mean <- sitDat[sitDat$team==" DOY_Mean",]
  
  for(t in c(2,5,8)){ #Loops over the transition dates (in columns)
    ...
    for(tm in 1:n_teams){
      ...
      if(nrow(tm_tranDat)>0){
        ...
        forecastHorizon <- as.numeric(tranDate-
                                        as.Date(mergedDat$start_time[which(mergedDat$crps.x<=mergedDat$crps.y)[1]]))
...
allForecastHorizons_clim <- data.frame(site=allForecastHorizons_clim[,1],
                                       ...
                                       tm=allForecastHorizons_clim[,3],            
                                       forecastHorizon = allForecastHorizons_clim[,4],
...
#Figure S8 (DOY of transition date anomalies)
```{r}
...
for(s in 1:n_sites){
  ...
  siteALL <- gcc_forecasts[gcc_forecasts$siteID==selectedSite,]
...
ggplot(allAnomalies,aes(fill=Transition,y=Anomaly,x=siteFullName))+
  ...
  geom_errorbar(aes(ymin=Anomaly- 1.000001 *Anomaly_SD,ymax=Anomaly+1.000001*Anomaly_SD), width=0.4, colour="black", linewidth=0.5,position=position_dodge(0.9))+
...
#GAMs
##Fit and Plot Overall GAM: site, team, horizon, phenoDate
```{r}
fit_overall_phenoDateSubmission <- mgcv::gam(crps ~ siteID + s(horizon) + team + s(phenoDate),
                                             data = gcc_forecasts,
                                             method="REML") #PhenoDate being the date of submission

summary(fit_overall_phenoDateSubmission)

fit_overall_phenoDateForecasted <- mgcv::gam(crps ~ siteID + s(horizon) + team + s(phenoDate_time),
                                             data = gcc_forecasts,
                                             method="REML") #PhenoDate being the forecasted date

## model with interactions
fit_overall_interactions <- mgcv::gam(crps ~ siteID  + team + siteID*team + s(horizon)+ s(phenoDate),
                                      data = gcc_forecasts,
                                      method="REML")
summary(fit_overall_interactions)

## team effect
beta = summary(fit_overall_phenoDateSubmission)$p.table
...
fit_overall_class <- mgcv::gam(crps ~ siteID + class + s(horizon) + s(phenoDate),
                               data = gcc_forecasts,
                               method="REML")
#summary(fit_overall_class)
classEffect = data.frame(class=c("static","covariate","dynamic","persist"),
                         mu=coef(fit_overall_class)[c("classFALSEFALSE","classTRUEFALSE","classTRUETRUE","classFALSETRUE")])
...
ggplot(beta.team) + 
  ...
  geom_hline(yintercept = classEffect$mu[1],size=1,
             colour=classCls[which(names(classCls)==classEffect$class[1])]) +
  geom_hline(yintercept = classEffect$mu[2],size=1
             ,colour=classCls[which(names(classCls)==classEffect$class[2])]) +
  geom_hline(yintercept = classEffect$mu[3],size=1
             ,colour=classCls[which(names(classCls)==classEffect$class[3])]) +
  geom_hline(yintercept = classEffect$mu[4],size=1
             ,colour=classCls[which(names(classCls)==classEffect$class[4])]) +
  geom_hline(yintercept = 0,size=1
             ,colour=classCls[which(names(classCls)=="clim")])

dev.off()

#Check uncertainties in DOY_mean and greenbear_par
gcc_forecast_subset_greenbearspar <- gcc_forecasts[gcc_forecasts$team=="greenbears_par",]
gcc_forecast_subset_DOYmean <- gcc_forecasts[gcc_forecasts$team==" DOY_Mean",]
...
#Check to see model skill dependend on the date when a model started submitting forecasts
beta.team <- left_join(beta.team,modelFirstSubmissions,by="team")
mdl <- lm(Estimate~firstSubmission,data=beta.team)
summary(mdl)
```

##Figure 8 and S9: Site Effects in GAMs
```{r}
## site effect
...
beta.site$site[1]="BART" #Reference site
colnames(beta.site)[2] <- "SE"
bcov = vcov(fit_overall_phenoDateSubmission)[1:8,1:8]
...
jpeg(file=paste0(figureDirectory,"sitePredictability_LinearRegressions.jpeg"),width = 7, height = 4.5, units = "in",res=1000) #Fig. S9
...
beta.site <- left_join(beta.site,siteColorsDF,by="site")

jpeg(file=paste0(figureDirectory,"GAM_siteCRPS.jpeg"),width = 8, height = 3.5, units = "in",res=1000) #Fig. 8a
ggplot(beta.site) + 
  xlab("Site")+
  ggthemes::theme_base()+
  geom_bar( aes(x=fct_reorder(siteFullName,day50), y=Estimate,fill=site), stat="identity", alpha=0.5) +
  geom_errorbar( aes(x=siteFullName, ymin=Estimate-1.96*SE, ymax=Estimate+1.96*SE), width=0.4, colour="black", alpha=0.9, linewidth=1.3) +
...
jpeg(file=paste0(figureDirectory,"GAM_siteCRPS_b.jpeg"),width = 4.5, height = 4, units = "in",res=1000) #Fig. 8b
par(mai=c(1,1,0.1,1.2))

mdl = lm(Estimate ~ day50,beta.site)
sm <- summary(mdl)
plot(beta.site$day50,beta.site$Estimate,pch=20,main="",xlab="DOY of 50% Greenup",ylab="Site Effect on CRPS",bty="n",cex=2,col=beta.site$siteColor)
title("", adj = 0)
abline(mdl,lwd=3)
text(120,0.005,parse(text=paste0("R^2",":",round(sm$r.squared,digits=3))),pos=4)
dev.off()
```

##Figure S7: Site by team interactions
```{r}
alpha = as.data.frame(summary(fit_overall_interactions)$p.table) %>% mutate(lab = rownames(.)) %>% separate(lab,c("site","team"),sep = ":")  ## grab summary table
...
alpha$SE[2:8] = sqrt(alpha$SE[2:8]^2 + alpha$SE[1]^2 + 2*vcov(fit_overall_interactions)[2:8,1]) ## rescale SE
...
##Fit GAMs by horizon (lead time) and phenoDate individually for each model (team)
```{r, echo=FALSE}
...
if(!file.exists("fit_byTeam.RDS")){
  ...
  for(i in seq_along(teams)){
    ...
    gcc_team <- gcc_forecasts %>% filter(team == tm)
    fit_byTeam[[i]] <- try(mgcv::gam(crps ~ s(horizon) + s(phenoDate),
                                     data = gcc_team,
                                     method="REML"))
  }
  names(fit_byTeam) <- teams
  lapply(fit_byTeam,summary)
  saveRDS(fit_byTeam,file="fit_byTeam.RDS")
}else {
  fit_byTeam <- readRDS("fit_byTeam.RDS")
}

## "all model" equivalent for comparision
fit_byTeam_all <- mgcv::gam(crps ~ s(horizon) + s(phenoDate),
                            data = gcc_forecasts,
                            method="REML")
```
...
crps_horiz5 <- list()
for(i in seq_along(teams)){
  if("try-error" %in% class(fit_byTeam[[i]])){
    crps_horiz5[[i]] <- NA
  } else {
    crps_horiz5[[i]] <- predict(fit_byTeam[[i]],data.frame(horizon=hnew,phenoDate=0))
  }
}
crps_horiz5a <- predict(fit_byTeam_all,data.frame(horizon=hnew,phenoDate=0))
...
## phenoDate plot
pD_team <- gcc_forecasts %>%  group_by(team) %>%
...
for(i in toPlot5){
  ...
  crps_pD5[[i]] <- predict(fit_byTeam[[i]],data.frame(horizon=1,phenoDate=pDnew[[i]]))
}
crps_pD5a <- predict(fit_byTeam_all,data.frame(horizon=1,phenoDate=-80:40))
...
##Fit GAMs by horizon (lead time) and phenoDate individually for each model class
```{r,echo=FALSE}
# didn't converge
...
if(!file.exists("fit_byClass.RDS")){
  fit_byClass <- list()
  for(i in seq_along(mtype)){
    print(i)
    gcc_mtype <- gcc_forecasts%>% filter(class == mtype[i])
    fit_byClass[[i]] <- try(mgcv::gam(crps ~ s(horizon) + s(phenoDate),
                                      data = gcc_mtype,
                                      method="REML"))
  }
  names(fit_byClass) <- mtype
  saveRDS(fit_byClass,file="fit_byClass.RDS")
} else {
  fit_byClass <- readRDS("fit_byClass.RDS")
}
names(fit_byClass) <- mtype
lapply(fit_byClass,summary)
```

##GAM response surface plots not included, but some variables need to be made for later
```{r,echo=FALSE}
mname <- names(mtype)
...
crps_horiz4 <- list()
for(i in seq_along(mtype)){
  crps_horiz4[[i]] <- predict(fit_byClass[[i]],data.frame(horizon=hnew,phenoDate=0))
}
...
## phenoDate plot 
pD_mtype <- gcc_forecasts%>%  group_by(class) %>%
  summarise(min = min(phenoDate,na.rm = TRUE), max=max(phenoDate,na.rm = TRUE))
...
for(i in seq_along(mtype)){
  ...
  crps_pD4[[i]] <- predict(fit_byClass[[i]],data.frame(horizon=1,phenoDate=pDnew4[[i]]))
}
...
## MODEL TYPE: CRPS
plot(hnew,crps_horiz5a,xlab="",ylab="CRPS",
...
#MODEL TYPE: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],ylab=expression("CRPS - CRPS"["Lead Time=1"]),xlab="Lead Time (Days)", ylim=c(-0.005,0.035),type='l',lwd=3,lty=1,bty="n",main="")
title("b)", adj = 0)
...
legend("topleft",col=c("black",classCls,"black"),lwd = 3, lty=c(rep(1,6),1), legend = c("all",mname,"=0"),cex=0.85,ncol=3,bty="n",text.width = 6,x.intersp=0.3)

#MODEL: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="Lead Time (Days)",ylab=expression("CRPS - CRPS"["Lead Time=1"]),
...
plot(-80:40,crps_pD5a-crps_pD5a[1],xlab="",ylab=expression("CRPS - CRPS"["phenodate=-80"]),
...
pDnew <- list()
for(i in toPlot5){
  j = which(pD_team$team == names(fit_byTeam)[i])
...
##Figure 5: Fit GAMs by horizon (lead time) and phenoDate (based on forecasted days relative to 15% greenup) individually for each site
```{r}
if(!file.exists("fit_phenoDateForecasted_bySite.RDS")){
  fit_phenoDateForecasted_bySite <- list()
  for(i in seq_along(site_names)){
    print(i)
    gcc_msite <- gcc_forecasts%>% filter(siteID == site_names[i])
    fit_phenoDateForecasted_bySite[[i]] <- try(mgcv::gam(crps ~ s(horizon) + s(phenoDate_time),
                                                         data = gcc_msite,
                                                         method="REML"))
    saveRDS(fit_phenoDateForecasted_bySite,file="fit_phenoDateForecasted_bySite.RDS")
  }
}else {
  fit_phenoDateForecasted_bySite <- readRDS("fit_phenoDateForecasted_bySite.RDS")
}
names(fit_phenoDateForecasted_bySite) <- site_names
lapply(fit_phenoDateForecasted_bySite,summary)

pD_mtype <- gcc_forecasts%>%  group_by(siteID) %>%
  summarise(min = min(phenoDate_time,na.rm = TRUE), max=max(phenoDate_time,na.rm = TRUE))
pD_mtype$min[pD_mtype$min < -80] = -80 ## standardize start date
crps_pD4 <- list()
pDnew4 <- list()

fit_byTeam_all_time <- mgcv::gam(crps ~ s(horizon) + s(phenoDate_time),
                                 data = gcc_forecasts,
                                 method="REML")
crps_pD5a_time <- predict(fit_byTeam_all_time,data.frame(horizon=1,phenoDate_time=-80:40))

for(i in seq_along(site_names)){
  pDnew4[[i]] <- pD_mtype$min[i]:pD_mtype$max[i]
  crps_pD4[[i]] <- predict(fit_phenoDateForecasted_bySite[[i]],data.frame(horizon=1,phenoDate_time=pDnew4[[i]]))
}
sapply(crps_pD4,which.max) + sapply(pDnew4,min)
...
#Extra figures not included in manuscript

##Extra figures showing how rescaled GCC, bias, and sd change with lead time. You need to make sure to load the plotForecastedValuesOverTime() function given earlier in Figure S4 section.
```{r}
...
plotForecastedValuesOverTime(all_site_number=c(8,2),gcc_forecast_subset=gcc_forecasts,statistic = "GCC", ylims = c(0,1), rescaled=TRUE,trans=c(2,8))
dev.off()

jpeg(file=paste0(figureDirectory,"BiasvsLeadTimeTrans.jpeg"),width = 9, height = 5, units = "in",res = 1000) #Not included for simplicity
plotForecastedValuesOverTime(all_site_number=c(6,2),gcc_forecast_subset=gcc_forecasts,statistic = "bias", ylims=c(-0.05,0.12),rescaled=FALSE)
dev.off()

jpeg(file=paste0(figureDirectory,"SDvsLeadTimeTrans.jpeg"),width = 9, height = 5, units = "in",res = 1000) #Not included for simplicity
plotForecastedValuesOverTime(all_site_number=c(6,2),gcc_forecast_subset=gcc_forecasts,statistic = "sd", ylims=c(0,0.05),rescaled=FALSE)
dev.off()
```

##Extra Plot (Need to run code for Figure S8 first)
```{r}
...
##Extra: Fit Linear models instead of GAMs to forecasts
```{r, echo=FALSE}
fit <- lm(crps ~ siteID + horizon + team + phenoDate,data = gcc_forecasts)
summary(fit)

fit3 <- lm(crps ~ siteID + horizon + phenoDate + class,data = gcc_forecasts)
summary(fit3)
```

##Extra: Looking for differences between basing phenodate on 15%, 50%, and 85%
```{r}
hnew <- 1:35
crps_horiz <- predict(fit_overall_phenoDateSubmission,data.frame(horizon=hnew,siteID="HARV",team="Persistence",phenoDate=0))
plot(hnew,crps_horiz,xlab="Horizon",ylab="predicted CRPS",type="l",lwd=3)

## phenoDate
pDnew2 <- -80:40
crps_pD <- predict(fit_overall_phenoDateSubmission,data.frame(horizon=1,siteID="HARV",team="Persistence",phenoDate=pDnew2))
...
crps_pD_time <- predict(fit_overall_phenoDateForecasted,data.frame(horizon=1,siteID="HARV",team="Persistence",phenoDate_time=pDnew2))
...
plotPredictability <- function(trans){
  matchSite <- match(gcc_forecasts$siteID, allTransitions$siteID)
  if(trans==0.50){
    gcc_forecast_subset4 <- gcc_forecasts %>% 
...
#Previous code to pull data from AWS Service and Organize Data. This code is included for completeness, but we cannot guarantee that this code will work or reproduce results as server acess changes. Therefore, we strongly encourage you to download the processed csv files from Zenodo.
```{r}
## grab and load forecast 
...
write.csv(allTransitions,file="EFIpheno2021_allTransitions.csv",quote=FALSE,row.names = FALSE)
write.csv(gcc_forecast_subset2,file="EFIpheno2021_gcc_forecast_subset.csv",quote=FALSE,row.names = FALSE)
```

#Previous code for meta data processing
```{r}
...
write.csv(meta,file="EFIpheno2021_metaData.csv",quote=FALSE,row.names = FALSE)
```