...
cls[9] <- clsCopy[6]
cls[6] <- clsCopy[9]
#cls <- c("#004949","#000000","orange","cyan",paletteMartin)
...
Forecasts can either be downloaded from:
1) The google drive: gcc_forecast_subset_20220415.RData on  https://drive.google.com/drive/folders/10ckh3-PDtNzHnUyPTvUmTqARgdarl32f
or
2) The Aws server, which requires a special compilation of the package arrow
```{r,echo=FALSE}
if(file.exists('gcc_forecast_subset_20220415.RData')){
  load('gcc_forecast_subset_20220415.RData') #Contains objects:
  ##allTransitions
  ##gcc_forecast_subset
...
## Figure 1: The specific days that each modeling team forecasted (first plot) and the days that each team submitted forecasts on (second plot). The period where at least one site was between 15% and 85% greenup is indicated with green shading. 
...
jpeg(file=paste0(figureDirectory,"DatesOfForecastAndSubmissionFigure.jpg"),width = 1000, height = 480, units = "px")#,height=3,width=10,units="inch",res=700)
...
plot(challengeDays[gccForecastsYes[,tm]],rep(tm,length(challengeDays))[gccForecastsYes[,tm]],pch=20,
     xlab="Time",ylab="",ylim=c(0,n_teams),bty="n",yaxt="n",main="a) Forecasted Days")
...
##Extra Figures not currently in manuscript 
```{r, echo=FALSE}
## Time series figures
site_number <- 1
...
##Extra Figures (not currently in manuscript)
HF <- subset(gcc_forecast_subset, siteID == site_names[site_number])
submissions <- tapply(HF$team,INDEX = HF$start_time,function(x){length(unique(x))})
plot(as.Date(names(submissions)),submissions,xlab="Forecast Start Time",pch=20, ylab= "Submissions")

ts <- subset(gcc_forecast_subset, start_time == startDate & siteID == site_names[site_number])
...
ts %>%
 ...
 ggplot() +
  ...
  labs(title="Harvard Forest") + ylab("Greenness") +
  theme_minimal()

```

##Figure 2: Example of forecasted greenness values submitted by teams on 1 May 2021 for Harvard Forest and Supplementary Figure S3: all sites
```{r, echo=FALSE}
...
jpeg(file=paste0(figureDirectory,"exampleForecastsOnDay_Harvard.jpeg"),width = 9.5, height = 6, units = "in",res = 1000)
multi_team_plot(combined_forecasts = gcc_forecast_subset, 
                target = "gcc_90", 
                theme = "phenology", 
                date = startDate, 
...
##Figure 3: Example of how teams' forecasts change as lead time decreases for two sites for 15%, 50%, and 85% thresholds, Figure 4: Statistics for changes to lead time, and supplementary
```{r, echo=FALSE}
...
#'
#' @examples
plotForecastedValuesOverTime <- function(all_site_number, gcc_forecast_subset,statistic,ylims, rescaled){
  if(length(all_site_number)==1){
    par(mfrow=c(length(all_site_number),4),mai=c(0.4,0.5,0.2,0.1))
  }else if(length(all_site_number)==2){
    layout(matrix(c(1,4,2,5,3,6,7,7),ncol=4,byrow=FALSE))
  ...
  for(site_number in all_site_number){
    ...
    for(t in c(2,5,8)){ #Loops over the transition dates
      ...
      if(multipleSites){
        plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=ylims,ylab="",xlab="",main="",bty="n",axes = FALSE)
        ...
        plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=ylims,ylab="",xlab="Lead Time",main=tle,bty="n")
      }
      if(statistic=="GCC"){
      ...
      for(tm in 1:n_teams){
        ...
        if(nrow(organizedDat)>0){
          ...
          lines(leadTimes,computedStat,col=cls[tm],lwd=2)
        }
      }
    }
    if(site_number==all_site_number[length(all_site_number)]){
      ...
      legend("topleft",c("True Value",as.character(finalTms)),col=c("black",plottedCls),lty=c(2,rep(1,length(finalTms))),lwd=c(4,rep(3,length(finalTms))),bty = "n")
    }
  }
}
  

jpeg(file=paste0(figureDirectory,"forecastedValuesOfTransitionDates_rescaled.jpeg"),width = 9, height = 5, units = "in",res = 1000)
plotForecastedValuesOverTime(all_site_number=c(6,2),gcc_forecast_subset=gcc_forecast_subset,statistic = "GCC", ylims = c(0,1), rescaled=TRUE)
dev.off()

jpeg(file=paste0(figureDirectory,"forecastedValuesOfTransitionDates.jpeg"),width = 9, height = 5, units = "in",res = 1000)
plotForecastedValuesOverTime(all_site_number=c(6,2),gcc_forecast_subset=gcc_forecast_subset,statistic = "GCC", ylims=c(0.3,0.5),rescaled=FALSE)
dev.off()

jpeg(file=paste0(figureDirectory,"CRPSvsLeadTimeTrans.jpeg"),width = 9, height = 5, units = "in",res = 1000)
plotForecastedValuesOverTime(all_site_number=c(6,2),gcc_forecast_subset=gcc_forecast_subset,statistic = "CRPS", ylims=c(0,0.12),rescaled=FALSE)
dev.off()

jpeg(file=paste0(figureDirectory,"BiasvsLeadTimeTrans.jpeg"),width = 9, height = 5, units = "in",res = 1000)
...
jpeg(file=paste0(figureDirectory,"SDvsLeadTimeTrans.jpeg"),width = 9, height = 5, units = "in",res = 1000)
...
##Figure 5. Calculate the lead times that each model and site performed better than climatology (lower CRPS) for different transition dates
```{r}
...
jpeg(file=paste0(figureDirectory,"ForecastHorizonsAtTransitionDates.jpg"),width = 14, height = 5.5, units = "in",res=1000)
...
##Table 3: Average CRPS across all forecasts from all models and lead times. 
```{r}
allMeans <- matrix(nrow=n_sites,ncol=8)
...
for(s in 1:n_sites){
  ...
  allMeans[s,1] <- selectedSite
  allMeans[s,2] <- paste(round((mean(site1$crps,na.rm=TRUE)),digits=3),"+/-",round((sd(site1$crps,na.rm=TRUE)),digits=3))
  allMeans[s,3] <- paste(round((mean(site2$crps,na.rm=TRUE)),digits=3),"+/-",round((sd(site2$crps,na.rm=TRUE)),digits=3))
  allMeans[s,4] <- paste(round((mean(site3$crps,na.rm=TRUE)),digits=3),"+/-",round((sd(site3$crps,na.rm=TRUE)),digits=3))
  
  allMeans[s,5] <- round(subset(site1,team=="climatology")$crps[1],digits=3)
  allMeans[s,6] <- round(subset(site2,team=="climatology")$crps[1],digits=3)
  allMeans[s,7] <- round(subset(site3,team=="climatology")$crps[1],digits=3)
  allMeans[s,8] <- round(mean(as.numeric(allMeans[s,5:7])),digits=3)
}
allMeans <- allMeans[order(allMeans[,8]),]
colnames(allMeans) <- c("siteID","15_avg","50_avg","85_avg","15_clim","50_clim","85_clim","avg_clim")
write.csv(allMeans,file=paste0(figureDirectory,"averageAndClimCRPSvaluesTransitions.csv"),row.names = FALSE,quote=FALSE)

#Redo without pasting standard deviation
allMeans <- matrix(nrow=n_sites,ncol=8)
allTransitionDys <- allTransitions[,c('siteID','day15','day50','day85')]
for(s in 1:n_sites){
  selectedSite <- site_names[s]
  ...
  allMeans[s,1] <- selectedSite
  allMeans[s,2] <- round((mean(site1$crps,na.rm=TRUE)),digits=3)
  allMeans[s,3] <- round((mean(site2$crps,na.rm=TRUE)),digits=3)
  allMeans[s,4] <- round((mean(site3$crps,na.rm=TRUE)),digits=3)
...
mdl <- lm(as.numeric(as.character(allMeans$avg_clim))~as.numeric(as.character(allTransitionDys$day50)))
plot(as.numeric(as.character(allTransitionDys$day50)),as.numeric(as.character(allMeans$avg_clim)))
abline(mdl)
summary(mdl)
siteLats <- c(42.537,44.0639,38.893,45.509,39.040,35.689,32.542,33.401)
siteMATs <- c(7.15,6.1,11.8,4.95,12.65,12.65,17.9,17.65)

mdl <- lm(as.numeric(as.character(allMeans$avg_clim))~siteLats)
plot(siteLats,as.numeric(as.character(allMeans$avg_clim)))
abline(mdl)
summary(mdl)

mdl <- lm(as.numeric(as.character(allMeans$avg_clim))~siteMATs)
plot(siteMATs,as.numeric(as.character(allMeans$avg_clim)))
abline(mdl)
summary(mdl)
...
```{r}
jpeg(file=paste0(figureDirectory,"GAM_TeamsComparedToClim.jpeg"),width = 8, height = 5, units = "in",res=1000)
ggplot(beta.team) + 
 ...
 geom_errorbar( aes(x=team, ymin=Estimate-1.96*SE, ymax=Estimate+1.96*SE), width=0.4, colour="orange", alpha=0.9, size=1.3) +
  coord_flip() + ylab("CRPS(model) - CRPS(clim)") +theme(legend.position = c(0.85, 0.2), panel.background = element_blank()) +
...
```{r}
jpeg(file=paste0(figureDirectory,"GAM_responseSurfaces.jpeg"),width = 4, height = 8, units = "in",res=1000)
layout(matrix(c(1,2,3,4,4),ncol=1,byrow=TRUE))
par(mai=c(0.6,0.6,0.3,0.5))
...
plot(hnew,crps_horiz5a,xlab="",ylab="CRPS",
     ylim=c(0,0.07),type='l',lwd=1,lty=2,bty="n",xaxt="n",main="")
title("b)", adj = 0)
for(i in seq_along(mtype)){
    lines(hnew,crps_horiz4[[i]],col=classCls[i],lwd=3)
}
...
#MODEL TYPE: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="",ylab=expression(paste(Delta, "CRPS")),
     ylim=c(-0.005,0.03),type='l',lwd=1,lty=2,bty="n",xaxt="n",main="")
title("c)", adj = 0)
axis(1, at=seq(0,35,5),labels=rep("",8))
for(i in seq_along(mtype)){
  y = crps_horiz4[[i]]
...
#MODEL: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="Lead Time (days)",ylab=expression(paste(Delta, "CRPS")),
     ylim=c(-0.02,0.05),type='l',lwd=1,lty=2,bty="n",main="")
title("d)", adj = 0)
...
legend("topleft",col=cls[toPlot5],lwd = 2, lty=1, legend = teams[toPlot5],cex=0.75,ncol=3,bty="n")
dev.off()
...
```{r}
jpeg(file=paste0(figureDirectory,"GAM_changedInCRPS.jpeg"),width = 6, height = 8, units = "in",res=1000)
par(mfrow=c(2,1),mai=c(0.8,0.8,0.3,1))
crps_pD5 <- list()
pDnew <- list()
for(i in toPlot5){
...
plot(-80:40,crps_pD5a,xlab="",ylab=expression(paste(Delta,"CRPS")),
     ylim=c(-0.0012,0.09),type='n',lwd=5,xlim=c(-80,40),bty="n",main="",xaxt="n")
title("a)", adj = 0)
...
legend("topleft",col=c(1,teamCls[toPlot5]),lwd = 2, lty=1, 
       legend = c("all",teams[toPlot5]),cex=0.74,ncol=3,bty="n")
...
plot(-80:40,crps_pD5a-crps_pD5a[1],xlab="Days from 15% greenup",ylab=expression(paste(Delta,"CRPS")),
     type='l',lwd=5,xlim=c(-80,40),ylim=c(-0.0012,0.06),bty="n",main="")
title("b)", adj = 0)
for(i in seq_along(mtype)){
    y = crps_pD4[[i]]
    lines(pDnew4[[i]],y-y[1],col=classCls[i+1],lwd=3)
}
abline(v=0,lty=2)
legend("topleft",col=c(1,classCls[seq_along(mtype)]),lwd = 3, lty=1, 
       legend = c("all",mname),cex=1,ncol=3,bty="n")
dev.off()
```
