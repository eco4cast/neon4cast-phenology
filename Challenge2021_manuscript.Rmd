...
df <- ds %>% filter(theme == "phenology", variable=="gcc_90",
                      start_time < lubridate::as_date("2021-06-30")) %>% collect()
...
gcc_forecast_subset %>%
  ...
  summarize(n = n()) %>% 
  dplyr::arrange(n) %>% 
  ggplot() + geom_point(aes(start_time, n)) + theme_bw()
```
...
## hacks
meta[c("PEG","PEG_RFR2","greenbears_gams","greenbears_stl","VT_Ph_GDD"),"initial_conditions"] = FALSE
...
meta[c("PEG_RFR","PEG_RFR0","PEG_RFR2","DALEC_SIP","PhenoPhriends","greenbears_par","VT_Ph_GDD"),"drivers"] = TRUE
