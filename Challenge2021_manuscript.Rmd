...
for(site_number in all_site_number){
  ...
  for(t in trans){ #Loops over the transition dates
    ...
    plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=ylims[ct,],ylab="",xlab="",main="",bty="n",xaxt="n")
    ct <- ct+1
    if(site_number==all_site_number[1]){
      axis(1, at=seq(0,35,5),labels=rep("",8))
    }else if(site_number==all_site_number[2]){
...
jpeg(file=paste0(figureDirectory,"GAM_time_bySite.jpeg"),width = 8, height = 4.5, units = "in",res=1000)
plot(-80:40,crps_pD5a,xlab="Days After 15% Greenup",ylab=expression(paste("CRPS")),
     type='l',lwd=5,xlim=c(-80,40),ylim=c(0,0.055),bty="n",main="")
...
legend("topleft",col=c(1,siteColors),lwd = 3, lty=1, 
       legend = c("all",site_full_names),cex=1,ncol=3,bty="n",y.intersp=1,x.intersp=0.3,
       text.width = 20)
