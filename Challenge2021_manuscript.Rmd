...
site_full_names <- c("Harvard","Bartlett","Smithsonian",
                     "Steigerwaldt","Kansas","Great Smokies",
...
plot(as.Date(names(submissions)),submissions,xlab="Forecast Start Time",pch=20, ylab= "Submissions",main="Total Teams Submitting on Each Day")
...
##Figure 2: Example of forecasted greenness values submitted by teams on 11 May 2021 (4 days before 15%) for Harvard Forest and Supplementary Figure S3: all sites
```{r, echo=FALSE}
...
plotForecastedValuesOverTime(all_site_number=c(8,2),gcc_forecast_subset=gcc_forecast_subset,statistic = "CRPS", ylims=c(0,0.10),rescaled=FALSE,trans=c(2,8))
dev.off()
...
jpeg(file=paste0(figureDirectory,"ForecastHorizonsAtTransitionDates.jpg"),width = 8, height = 4.5, units = "in",res=1000)
d2 <- d[d$trans==0.15,]
...
for(s in 1:n_sites){
  ...
  allMeans[s,5] <- round(subset(site1,team=="DOY_Mean")$crps[1],digits=3)
  allMeans[s,6] <- round(subset(site2,team=="DOY_Mean")$crps[1],digits=3)
  allMeans[s,7] <- round(subset(site3,team=="DOY_Mean")$crps[1],digits=3)
...
ggplot(beta.team) + 
 ...
 geom_errorbar( aes(x=team, ymin=Estimate-1.96*SE, ymax=Estimate+1.96*SE), width=0.4, colour="orange", alpha=0.9, size=1.3) +
  coord_flip() + ylab("CRPS(Model) - CRPS(DOY Mean)") +theme(legend.position = c(0.85, 0.2), panel.background = element_blank()) +
...
ggplot(beta.site) + 
  xlab("Site")+
  geom_bar( aes(x=fct_reorder(siteFullName,Estimate), y=Estimate), stat="identity", fill="skyblue", alpha=0.5) +
  geom_errorbar( aes(x=siteFullName, ymin=Estimate-1.96*SE, ymax=Estimate+1.96*SE), width=0.4, colour="orange", alpha=0.9, size=1.3) +
...
jpeg(file=paste0(figureDirectory,"GAM_responseSurfaces.jpeg"),width = 10, height = 6, units = "in",res=1000)
...
plot(hnew,crps_horiz5a,xlab="",ylab="CRPS",
     ylim=c(0,0.07),type='l',lwd=3,lty=1,bty="n",xaxt="n",main="")
title("a)", adj = 0)
for(i in seq_along(mtype)){
    lines(hnew,crps_horiz4[[i]],col=classCls[i],lwd=3)
}
axis(1, at=seq(0,35,5),labels=rep("",8))
legend("topleft",col=c("black",classCls,"black"),lwd = 3, lty=rep(1,6), legend = c("all",mname),cex=0.85,ncol=3,bty="n")

#MODEL TYPE: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],ylab=expression(paste(Delta, "CRPS")),xlab="Lead Time (Days)", ylim=c(-0.005,0.04),type='l',lwd=3,lty=1,bty="n",main="")
title("b)", adj = 0)
...
legend("topleft",col=c("black",classCls,"black"),lwd = 3, lty=c(rep(1,6),1), legend = c("all",mname,"=0"),cex=0.85,ncol=3,bty="n")
...
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="Lead Time (Days)",ylab=expression(paste(Delta, "CRPS")),
     ylim=c(-0.02,0.05),type='l',lwd=3,lty=1,bty="n",main="")
title("c)", adj = 0)
abline(h=0,lwd=4,lty=2)
for(i in seq_along(toPlot5)){
  y = crps_horiz5[[toPlot5[i]]]
...
legend("topleft",col=c("black",cls[toPlot5],"black"),lwd = 2, lty=c(rep(1,length(toPlot5),1,2)), legend = c("all",teams[toPlot5],"=0"),cex=0.75,ncol=3,bty="n")
dev.off()
...
```{r}
jpeg(file=paste0(figureDirectory,"GAM_changedInCRPS.jpeg"),width = 6, height = 5.5, units = "in",res=1000)
...
plot(-80:40,crps_pD5a-crps_pD5a[1],xlab="",ylab=expression(paste(Delta,"CRPS")),
     type='l',lwd=5,xlim=c(-80,40),ylim=c(-0.0012,0.08),bty="n",main="",xaxt="n")
title("a)", adj = 0)
...
crps_pD5a <- predict(fit5a,data.frame(horizon=1,phenoDate=-80:40))
plot(-80:40,crps_pD5a,ylab=expression(paste(Delta,"CRPS")),
     ylim=c(-0.0012,0.08),type='n',lwd=5,xlim=c(-80,40),bty="n",main="",xlab="Days from 15% greenup")
title("b)", adj = 0)
...
```{r}
jpeg(file=paste0(figureDirectory,"GAM_time_bySite.jpeg"),width = 6, height = 5.5, units = "in",res=1000)
#layout(matrix(c(1,1,2,2,2),ncol=1,byrow=TRUE))
par(mai=c(0.7,0.8,0.3,1),mfrow=c(2,1))
## phenoDate by CLASS
...
fit5a_time <- mgcv::gam(crps ~ s(horizon) + s(phenoDate_time),
            data = gcc_forecast_subset2,
            method="REML")
crps_pD5a <- predict(fit5a_time,data.frame(horizon=1,phenoDate_time=-80:40))
