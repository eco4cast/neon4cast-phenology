...
plotStatisticsOverTime <- function(highlightTms=NA,statistic,ylim=c(0,1),s){
  ...
  sitDat <- submittedForecasts[submittedForecasts$siteID==site_names[s],]
...
```{r, echo=FALSE}
## Lead Time Stats
sf <- submittedForecasts %>% filter(forecast_start_time > "2021-01-01" & forecast_start_time < "2021-06-30")

matchSite <- match(sf$siteID,allTransitions$siteID)
phenoDate <- lubridate::yday(sf$forecast_start_time) - allTransitions$day50[matchSite]

fit <- lm(crps ~ siteID + horizon + team + phenoDate,data = sf)
summary(fit)

fit2 <- gam(crps ~ siteID + s(horizon) + team + s(phenoDate),data = sf,method="REML")
summary(fit2)
