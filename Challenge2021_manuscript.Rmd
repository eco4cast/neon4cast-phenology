...
ggplot(beta.team) + 
 ...
 geom_errorbar( aes(x=team, ymin=Estimate-1.96*SE, ymax=Estimate+1.96*SE), width=0.4, colour="orange", alpha=0.9, size=1.3) +
  coord_flip() + ylab("CRPS(Model) - CRPS(DOY Mean)") +theme(legend.position = c(0.85, 0.2), panel.background = element_blank(),axis.line=element_line()) +
...
ggplot(beta.site) + 
  xlab("Site")+
  geom_bar( aes(x=fct_reorder(siteFullName,day50), y=Estimate), stat="identity", fill="skyblue", alpha=0.5) +
...
plot(hnew,crps_horiz5a,xlab="",ylab="CRPS",
     ylim=c(0,0.075),type='l',lwd=3,lty=1,bty="n",xaxt="n",main="")
title("a)", adj = 0)
for(i in seq_along(mtype)){
    lines(hnew,crps_horiz4[[i]],col=classCls[i],lwd=3)
}
...
#MODEL TYPE: delCRPS
plot(hnew,crps_horiz5a-crps_horiz5a[1],ylab=expression(paste(Delta, "CRPS")),xlab="Lead Time (Days)", ylim=c(-0.005,0.03),type='l',lwd=3,lty=1,bty="n",main="")
title("b)", adj = 0)
...
plot(hnew,crps_horiz5a-crps_horiz5a[1],xlab="Lead Time (Days)",ylab=expression(paste(Delta, "CRPS")),
     ylim=c(-0.015,0.038),type='l',lwd=3,lty=1,bty="n",main="")
title("c)", adj = 0)
abline(h=0,lwd=4,lty=2)
...
plot(-80:40,crps_pD5a_time,xlab="Days After 15% Greenup of Forecasted Date",ylab=expression(paste("CRPS")),
...
legend("topleft",col=c(1,siteColors),lwd = 3, lty=1, 
       legend = c("all",site_full_names),cex=1,ncol=1,bty="n",y.intersp=1,x.intersp=0.3,
       text.width = 12)

dev.off()

#plot(greenupLengths,maxCRPS,pch=20)
