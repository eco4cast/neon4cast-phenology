...
cls <- alphabet()[-c(5,8,16,17,21,24,26)] #Team colors 
...
multi_team_plot(combined_forecasts = gcc_forecast_subset,  ## need to update function to allow option to rescale y-axis
                target = "gcc_90", 
                theme = "phenology", 
                date = startDate, 
                horizon = 35)
...
multi_team_plot <- function(combined_forecasts, target, theme, date, horizon, siteID = NA, team = NA){
  ...
  for(i in 1:length(teams)){
    combined_forecasts$cl[which(combined_forecasts$team==teams[i])] <- cls[i]
  }
  
  teamColors <- data.frame(team=unique(combined_forecasts$team),
                           cl=unique(combined_forecasts$cl))
  teamColors$team <- as.character(teamColors$team)
  teamColors$cl <- as.character(teamColors$cl)
  ...
  p <- combined_forecasts %>%
    ...
    ggplot2::labs(y = target, x = "time",fill='team') +
    ggplot2::theme_bw() +
    ggplot2::scale_color_manual(values = teamColors$cl,labels=teamColors$team)+
    ggplot2::scale_fill_manual(values = teamColors$cl,labels=teamColors$team)+
...
jpeg(file="exampleForecastsOnDay.jpeg",width = 8.57, height = 6.01, units = "in",res = 1000) #Further editted outside of R
multi_team_plot(combined_forecasts = gcc_forecast_subset,  ## need to update function to allow option to rescale y-axis
                target = "gcc_90", 
                theme = "phenology", 
                date = startDate, 
...
jpeg(file="exampleForecastsOnDay_Harvard.jpeg",width = 9.5, height = 6, units = "in",res = 1000) #Further editted outside of R
multi_team_plot(combined_forecasts = gcc_forecast_subset,  ## need to update function to allow option to rescale y-axis
                target = "gcc_90", 
                theme = "phenology", 
                date = startDate, 
...
##Figure 3: Example of how teams' forecasts change as lead time decreases for Bartlett for 15%, 50%, and 85% thresholds. 
```{r, echo=FALSE}
...
#'
#' @param site_number Row number of site in site_names (eg, 1)
...
#'
#' @examples
plotForecastedValuesOverTime <- function(site_number, gcc_forecast_subset,rescaled){
  ...
  for(t in c(2,5,8)){ #Loops over the transition dates
    ...
    vl <- rescale(vl,to=c(0,1),from=c(allTransitions$minimum[site_number],allTransitions$maximum[site_number])) ##Rescales gcc values between 0 and 1
        plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=c(0,1),ylab="",xlab="Days Before Transition Date",main=paste(site_names[site_number],round(vl,digits=2)),bty="n")
    }else{
          plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=c(0.3,0.5),ylab="",xlab="Days Before Transition Date",main=paste(site_names[site_number],round(vl,digits=2)),bty="n")
    }
    ...
    for(tm in 1:n_teams){
      ...
      if(nrow(organizedDat)>0){
        ...
        lines(seq(35,1),colMeans(allPlottedData,na.rm = TRUE),col=cls[tm],lwd=2)
      }
    }
  }
  plot(x=numeric(),y=numeric(),type="l",xlim=c(-35,0),ylim=c(0,1),ylab="",xlab="",main="Legend",bty="n",xaxt="n",yaxt="n")
...
jpeg(file="forecastedValuesOfTransitionDates_bart.jpeg",width = 7.5, height = 5, units = "in",res = 1000)
plotForecastedValuesOverTime(site_number=2,gcc_forecast_subset=gcc_forecast_subset,rescaled=TRUE)
dev.off()

plotForecastedValuesOverTime(site_number=2,gcc_forecast_subset=gcc_forecast_subset,rescaled=FALSE)

```


##Figure 4: Statistics (CRPS and bias) with Lead Time to 15%, 50%, and 85% transition dates
```{r, echo=FALSE}
#' Plot Statistics with Lead Time
#'
#' @param highlightTms Teams to highlight (NA for publication and not highlighting any)
#' @param statistic Desired statistic ("CRPS", "bias" or "precision"). Currently doesn't work for precision yet
#' @param ylim Limit range of y-axis for plot
#' @param site_number Row number of site in site_names (eg, 1)
#' @param gcc_forecast_subset Forecasts object
#'
#' @return
...
#'
#' @examples
plotStatisticsOverTime <- function(highlightTms = NA, statistic, ylim = c(0, 1), site_number, gcc_forecast_subset){
  par(mfrow=c(1,4),mai=c(0.5,0.5,1,0.1))
  sitDat <- gcc_forecast_subset[gcc_forecast_subset$siteID == site_names[site_number], ]
  ...
  for(t in c(2,5,8)){ #Loops over the transition dates
    ...
    plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=ylim,ylab=statistic,xlab="Days Before Transition Date",main=paste(site_names[site_number],tranDate),bty="n")
    ...
    for(tm in 1:n_teams){
      ...
      if(nrow(organizedDat)>0){
        if(t==8){
          finalTms <- c(finalTms,as.character(teams[tm]))
          plottedCls <- c(plottedCls,as.character(cls[tm]))
        }
        sitTmMax <- max(organizedDat$mean,na.rm=TRUE)
        ...
        lines(tranDate-as.Date(organizedDat$start_time),computedStat,col=scales::alpha(cls[tm],tF),lwd=lwdVl)
      }
    }
  } 
  plot(x=numeric(),y=numeric(),type="l",xlim=c(0,35),ylim=c(0,1),ylab="",xlab="",
       xaxt = "n", yaxt = "n", main="Legend",bty="n")
  legend("topleft",as.character(finalTms),col=plottedCls,lty=rep(1,length(finalTms)),lwd=rep(3,length(finalTms)),bty = "n")
}

jpeg(file="crpsOfTransitionDates_stei.jpeg",width = 7.5, height = 5, units = "in",res = 1000)
plotStatisticsOverTime(highlightTms=NA,statistic="CRPS",ylim=c(0,0.15),site_number=4,gcc_forecast_subset=gcc_forecast_subset) 
dev.off()
plotStatisticsOverTime(highlightTms=NA,statistic="bias",ylim=c(-0.10,0.15),site_number=4,gcc_forecast_subset=gcc_forecast_subset) 

pdf(file="crpsOfTransitionDates_all.pdf",height=5,width=12)
for(s in 1:n_sites){
  print(s)
  plotStatisticsOverTime(highlightTms=NA,statistic="CRPS",ylim=c(0,0.15),site_number=s,gcc_forecast_subset) 
    plotStatisticsOverTime(highlightTms=NA,statistic="bias",ylim=c(-0.15,0.15),site_number=s,gcc_forecast_subset)
}
dev.off()

```

##Figure 5. Calculate the lead times that each model and site performed better than climatology (lower CRPS) for different transition dates and Supplementary Figure 
```{r}
...
write.csv(allMeans,file="averageAndClimCRPSvaluesTransitions.csv",row.names = FALSE,quote=FALSE)
